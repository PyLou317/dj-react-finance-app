...
@shared_task
def initial_sync_simplefin():
    access_url = os.environ.get('SIMPLEFIN_ACCESS_URL')
    if not access_url:
        return "No ACCESS_URL"

    # 1. Get transactions from the last 365 days
    duration = int(time.time()) - (365 * 24 * 60 * 60)
        ...
        with transaction.atomic():
            for acc_data in data.get('accounts', []):
                ...
                account, _ = Account.objects.update_or_create(
                    ...
                    defaults={
                        'org': org,
                        'user_id': 'lucas_dev', # Use your actual identifier here
                        'name': acc_data['name'],
...
def sync_simplefin():
    ...
    duration = int(time.time()) - (2 * 24 * 60 * 60)
    sync_url = f"{access_url}/accounts?start-date={duration}"
